...
[Datos de delitos y policías por pueblo](https://raw.githubusercontent.com/smramirezb/datos_ejemplos/refs/heads/main/dat_pueblos.txt)
...
- **Hipótesis nula (\(H_0\))**: \( \rho = 0 \)  
  *(No existe correlación en la población, es decir, las variables no están linealmente asociadas)*.  
...
4. **Independencia de las observaciones**: Cada par de observaciones \((X_i, Y_i)\) debe ser **independiente** de las demás. 
...
Los resultados del test de normalidad de Shapiro-Wilk para ambas variables no rechazan la hipotesis de normalidad con una significancia del 5%, valor-p = 0.264 valor-p = 0.9178 son mayores a 0.05.
