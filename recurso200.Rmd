...
$$
Y_i \mid X_i \sim N(E[Y_i | X_i], \sigma^2), \quad E[Y_i | X_i] = \beta_0 + \beta_1 X_i, \quad \sigma^2 = \text{constante}
$$
...
- **El supuesto de normalidad de la variable \( Y \)**: La distribución de \( Y \) indica un comportamiento aproximadamente normal.

- **La dependencia de la media \( \mu \) con \( X \)**: Se observa  la relación entre \( X \) y \( Y \) es lineal.

-  **La varianza constante (homocedasticidad)**: Se analiza que la dispersión de los valores de \( Y \) respecto a \( X \) permanece constante a lo largo de toda la escala de \( X \).
...
Por otro lado, en la **Figura 3.12**, la distribución de la curva **roja** no sigue una normal, sino que muestra un comportamiento más parecido a una **distribución exponencial**. Esta condición puede afectar la validez de los estimadores en un modelo de regresión, ya que se incumple el supuesto de normalidad.
...
for (i in seq_along(selected_x)) {
  ...
  if (x_val == 10) {  # Aplicar la distribución exponencial solo para X = 10
    ...
    density <- dexp(y_vals - min(y_vals) + 1, rate = rate)  # Evitar valores negativos
    density <- density / max(density) * 5  # Escalar la densidad para mejor visualización
  } else {
...
```
**Figura 3.13** Relación entre Resistencia y Edad.
</center>
<br/><br/>
...
- **Intercepto estimado (\(\hat{\beta}_0 = 2627.82\))**: La resistencia esperada de la soldadura cuando la **edad es cero** es de 2627.82 psi.

- **Pendiente estimada (\(\hat{\beta}_1 = -37.15\))**: Indica que, por cada **semana de aumento en la edad de la soldadura**, la resistencia **disminuye en promedio en 37.15 psi**.

- **Desviación estándar estimada de los errores (\(\hat{\sigma} = 96.11\))**: La dispersión de los residuos en torno a la línea de regresión es de 96.11 psi. 

A partir de estos valores, el modelo de regresión se puede expresar de dos maneras:

 1. En términos de la distribución de \( Y \) 
...
Matemáticamente, los valores ajustados en el modelo de regresión se expresan como:
...
Matemáticamente, los **residuales (\(e_i\))** en un modelo de regresión lineal se definen como:

\[
\widehat{\varepsilon}_i =e_i= Y_i - \widehat{Y}_i
\]

Los **residuales son los estimadores del error**. El objeto `mod1$residuals` en **R** almacena la diferencia entre los valores observados y los valores ajustados del modelo. El análisis de los residuales es fundamental para validar los supuestos del modelo de regresión  y garantizar que las inferencias obtenidas sean confiables.
