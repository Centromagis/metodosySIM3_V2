...
Estos valores observados se utilizan para **inferir los parámetros desconocidos** \( \beta_0, \beta_1, \sigma^2 \) mediante métodos como **máxima verosimilitud (EMV)**.
...
En el modelo de regresión lineal, se asume que los **errores \( \varepsilon_i \) son independientes e identicamente distribuidos (i.i.d)**, es decir:
...
- **Las \( y_i \) y \( x_i \) representan valores observados** en la muestra.
...
<div class="caja-nota">
> *En la práctica, en la regresión lineal clásica basada en MCO y EMV, se trata a  \( X \)  como fija y a  \( Y \)
como aleatoria. Sin embargo, en ciertos escenarios, especialmente en problemas de inferencia Bayesiana o en modelos con errores en variables, ambas pueden considerarse aleatorias y se deben usar métodos específicos para la estimación.*
</div>
