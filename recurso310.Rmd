...
Cuando los residuales estandarizados **siguen una distribución normal**, se espera que aproximadamente el **95% de los valores estandarizados estén dentro del rango \( [-2,2] \)**, y el **99% dentro del rango \( [-3,3] \)**. Valores fuera de estos rangos pueden indicar **posibles valores atípicos o problemas de ajuste** en el modelo.
...
Si un modelo lineal está **correctamente especificado**, los **residuales de Pearson** deben cumplir las siguientes condiciones:

1. **Independencia con los valores ajustados**: Los residuales de Pearson no deben mostrar patrones sistemáticos en función de los valores ajustados \( \hat{y}_i \).

2. **Independencia con los predictores**: La distribución de los residuales de Pearson no debe depender de los valores de las variables predictoras \( x_i \).

3. **Comportamiento aleatorio**: Los residuales deben dispersarse aleatoriamente alrededor de cero sin formar patrones estructurados.

Si estas condiciones se cumplen, los gráficos de residuales generados deben ser **"gráficos nulos"**, es decir, **gráficos sin características sistemáticas**. Esto significa que la **distribución condicional de los residuos** en el eje vertical del gráfico **no debe cambiar** con respecto a los valores ajustados o con una variable predictora en el eje horizontal.
...
**Casos especiales**: Por ejemplo, si todos los pesos son iguales** (\( w_i = 1 \) para todas las observaciones), la regresión ponderada **equivale a la regresión lineal ordinaria**.
