...
La **matriz hat** (o **matriz sombrero**) es una herramienta fundamental en la regresión lineal, ya que permite medir la influencia que tiene cada observación en los valores ajustados del modelo. Matemáticamente, el leverage se define como la diagonal de la matriz  \( H \), determinada por la **matriz de diseño**  \( X \) como sigue:
  ...
  donde los elementos de la diagonal se determinan como sigue,
  
  \[
h_{ii} = \frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_{j=1}^{n} (x_j - \bar{x})^2}
\]

La cantidad \( h_{ii} \) se denomina **leverage** y corresponde al elemento \( i \)-ésimo de la diagonal de la matriz **sombrero** \( H \).  Estos valores indican **qué tan alejados están los puntos individuales en el espacio de las variables predictoras** con respecto al centro de la nube de datos. Además, indican qué tan influyente es una observación en la estimación de los valores ajustados del modelo.

En la **Figura 3.44** se ilustra este concepto en un caso con tres variables explicativas \( X_1, X_2 \) y \( X_3 \) que determinan la matriz de diseño  \( X \). En ella, el centroide de los datos se representa con un **punto rojo**, y se observan las distancias relativas de cada observación con respecto a este centroide.
...
<div class="caja-nota">
<h3>Nota:</h3>
>*En esta unidad, el análisis se centra en la **Regresión Lineal Simple**, que permite modelar la relación entre una **única variable explicativa cuantitativa** y una **variable respuesta cuantitativa**. Sin embargo, en muchos casos, los fenómenos de interés involucran **múltiples factores o variables cualitativas** que afectan la variable respuesta. Por ello, en el curso de **Modelos Estadísticos para la Toma de Decisiones**, que se estudiará el siguiente semestre, se explorará la **Regresión Lineal Múltiple**. Este modelo amplía las herramientas de análisis al considerar **varias variables explicativas simultáneamente**.*  
</div>
...
[Datos de medidas antropométricas](https://raw.githubusercontent.com/Centromagis/metodosySIM3_V2/refs/heads/main/datos_MetySim/dat_atip.txt)
...
file <- "https://raw.githubusercontent.com/Centromagis/metodosySIM3_V2/refs/heads/main/datos_MetySim/dat_atip.txt"
...
\[
\hat{\text{Peso}} = -37.1552 + 2.9002 \times \text{Circunferencia del cuello}\\ \varepsilon_i \sim N(0, \hat{\sigma}^2=5.936^2)
\]
...
Las **Figuras 3.49** muestran que las observaciones **11 y 51** presentan los valores más altos de **distancia de Cook**. Sin embargo, en principio se puede decir que dichos valores son inferiores a 1, lo que sugiere que no ejercen una influencia significativa en el ajuste del modelo.
...
Los valores de los **residuos estandarizados**, **leverage** y **distancia de Cook** se presentan a continuación. Los resultados indican que **las observaciones 8, 11 y 51 registran los valores mayores al resto** en estas métricas. 
